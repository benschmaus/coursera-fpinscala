 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of a few numbers") {
    ...
    assert(sum(List(1, 2, 2, 2, 2, 10, 2, 2, 2)) === 25)
    assert(sum(List()) === 0)
  ...
  test("max of a few numbers") {
    ...
    assert(max(List(3, 7, 2, 92, 1, 1, 1, 1, 127, 127, 1076239)) === 1076239)
    intercept[NoSuchElementException] {
      assert(max(List()) === 0)
    }
    assert(max(List(3, 7, 2, 92, 1, 1, 1, -22, 1, 127, 127, 1076239)) === 1076239)
  }
